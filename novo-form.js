...
const months = [1]; // Exemplo: [1, 2] para Janeiro e Fevereiro
...
function generateMonthlyForms() {
  months.forEach((m) => {
    const form = FormApp.create(`Jornada do Novo - Escala de ${MONTH_NAMES[m - 1]}`);
    form.setDescription(description);
    form.setConfirmationMessage(confirmationMessage);
    setupFormForMonth(form, m);
    // Exibe algumas informações e move o Form para a pasta desejada
    Logger.log(`Form Name: ${form.getTitle()}`);
    Logger.log(`Form Link: ${form.getEditUrl()}`);
    const folderName = "Meu Drive/Igreja/Jornada do Novo/Escalas/2025";
    const folders = DriveApp.getFoldersByName(folderName);
    if (folders.hasNext()) {
      const folder = folders.next();
      folder.addFile(DriveApp.getFileById(form.getId()));
    } else {
      Logger.log("Pasta não encontrada: " + folderName);
    }
    setupOnFormSubmitTrigger(form); // Chama a função passando o form
  });
}

function setupFormForMonth(form, m) {
  form.getItems().forEach(item => form.deleteItem(item));
  form.setTitle(`Jornada do Novo - Escala de ${MONTH_NAMES[m - 1]}`);
  form.setDescription(description);
  form.setConfirmationMessage(confirmationMessage)
  ...
  var choices = sections.map((section, index) => {
    ...
    updateSectionQuestion(form, section, year, m);
...
function updateSectionQuestion(form, section, year, m) {
  ...
  var choices = findWeekendsWithOptions(year, m, section.days, section.totalSlots);
...
function findWeekendsWithOptions(year, m, days, totalSlots) {
  var options = [];
  var date = new Date(year, m - 1, 1);
...
function updateFormOptions() {
  ...
  try {
      ...
      if (e.message === 'The form currently has no response destination.') {
          ...
          // Move a planilha para a pasta desejada e exibe o link
          const folderName = "Meu Drive/Igreja/Jornada do Novo/Escalas/2025";
          const folders = DriveApp.getFoldersByName(folderName);
          if (folders.hasNext()) {
            const folder = folders.next();
            folder.addFile(DriveApp.getFileById(destinationId));
            Logger.log("Spreadsheet Link: " + newSpreadsheet.getUrl());
          } else {
            Logger.log("Pasta não encontrada: " + folderName);
          }
...
function setupOnFormSubmitTrigger(form) {
  var triggers = ScriptApp.getProjectTriggers();
  // Remove qualquer gatilho existente para evitar duplicação no mesmo form
  triggers.forEach(function(trigger) {
    if (trigger.getHandlerFunction() === 'onFormSubmit' && trigger.getTriggerSourceId() === form.getId()) {
      ScriptApp.deleteTrigger(trigger);
    }
  });
  // Associa o gatilho especificamente ao form passado
